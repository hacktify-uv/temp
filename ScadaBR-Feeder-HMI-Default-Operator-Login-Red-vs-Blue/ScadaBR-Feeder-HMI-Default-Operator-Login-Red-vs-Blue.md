# ScadaBR Feeder HMI Default Operator Login Red vs Blue Writeup
...
<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/red/media/image1.png" title="Figure 1" style="width:6.55in;height:4.90611in" alt="Figure 1. Kali reconnaissance showing the target services and the HTTP redirect toward the ScadaBR application." />
...
<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/red/media/image2.png" title="Figure 2" style="width:6.55in;height:6.14148in" alt="Figure 2. Root HTTP response and robots.txt disclosure revealing /engineering-docs/commissioning/." />
...
<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/red/media/image3.png" title="Figure 3" style="width:6.55in;height:4.60903in" alt="Figure 3. Engineering archive enumeration exposing the Feeder 01 commissioning handover record." />
...
<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/red/media/image4.png" title="Figure 4" style="width:6.55in;height:5.76303in" alt="Figure 4. Commissioning handover record disclosing the feeder.operator account and unchanged initial password." />
...
<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/red/media/image5.png" title="Figure 5" style="width:6.55in;height:3.40122in" alt="Figure 5. ScadaBR authentication using the credential recovered from the exposed handover record." />
...
<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/red/media/image6.png" title="Figure 6" style="width:6.55in;height:3.36038in" alt="Figure 6. Authorized baseline before exploitation: command CLOSE, status CLOSED, and feeder load approximately 315 A." />
...
<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/red/media/image7.png" title="Figure 7" style="width:6.55in;height:3.22383in" alt="Figure 7. Confirmed exploited state: breaker command OPEN, breaker status OPEN, and feeder current reduced to 3 A." />
...
<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/red/media/image8.png" title="Figure 8" style="width:6.55in;height:3.86592in" alt="Figure 8. Packaged attack script demonstrating discovery-only credential recovery and authenticated restore through the ScadaBR workflow." />
...
| **LIVE WIRESHARK NETWORK ACTIVITY VALIDATION** |
|------------------------------------------------|

**ScadaBR â€” Feeder HMI Default Operator Login**

Red Team attack execution and server-side packet observation

| **Attacker system** | Kali Linux Live USB |
|----|----|
| **Target system** | Ubuntu 22.04 LTS running Nginx, ScadaBR/Tomcat and the Modbus feeder process |
| **Observed addresses** | Attacker-side source: 172.24.4.1; target: 203.0.0.42 |
| **Relevant ports** | TCP/80 (external HTTP), TCP/8080 (internal ScadaBR/Tomcat), TCP/1502 (internal Modbus/TCP) |
| **Capture method** | Wireshark running on the target server through an RDP graphical session; interface: any |

**Objective**

Demonstrate that the supplied Red Team script generates observable network activity on the target server, invokes the authenticated ScadaBR breaker-control endpoint, receives a successful server response, and operates alongside a live Modbus/TCP process.

| **Validation outcome:** The authenticated DWR setPoint request and HTTP 200 response were captured live. The server also showed continuous Modbus/TCP polling on localhost TCP/1502. |
|----|

# **1. Test Procedure**

1.  Opened the Ubuntu server desktop through RDP and started Wireshark as the ubuntu user.

2.  Selected the Linux any interface to capture both external and loopback traffic.

3.  Applied the display filter: tcp.port == 80 \|\| tcp.port == 8080 \|\| tcp.port == 1502.

4.  Executed the packaged Red-Team-Attack-Script.sh from the Kali terminal.

5.  Reviewed the HTTP request, followed the TCP stream and decoded TCP/1502 as Modbus/TCP.
...
| **Credential discovery** | The commissioning operator username and password were extracted from the exposed engineering handover document. |
|----|----|
| **Authentication** | ScadaBR accepted the discovered operator credential. |
| **Control action** | The script sent an authenticated OPEN request to BREAKER_52A_CMD through the ScadaBR DWR endpoint. |
| **Remote verification note** | Direct Modbus verification from Kali was unavailable because TCP/1502 is intentionally bound to server loopback. |

# **2. Live Capture Overview**

<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/blue/media/image1.png" style="width:9.45in;height:5.09588in" />

*Figure 1 â€” Wireshark display filter showing the external HTTP attack flow and relevant internal service ports.*


**Observed**
â€¢ The filter covers TCP/80, TCP/8080 and TCP/1502.

â€¢ The attack traffic is visible from 172.24.4.1 to 203.0.0.42 over TCP/80.

â€¢ The packet list includes an HTTP POST to the ScadaBR DataPointDetailsDwr.setPoint.dwr endpoint.


# **3. Authenticated ScadaBR Control Request**

<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/blue/media/image2.png" style="width:9.45in;height:4.4239in" />

*Figure 2 â€” Packet details for the authenticated HTTP POST request to the ScadaBR DWR point-control endpoint.*


**Observed**
â€¢ Request method: POST.

â€¢ Request URI: /ScadaBR/dwr/call/plaincall/DataPointDetailsDwr.setPoint.dwr.

â€¢ The request includes an authenticated ScadaBR session cookie; the transient identifier is redacted in this document.


# **4. TCP Stream and Server Response**

<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/blue/media/image3.png" style="width:9.45in;height:4.58137in" />

*Figure 3 â€” Follow TCP Stream output showing the DWR method, point parameters and successful HTTP response.*


**Observed**
â€¢ c0-scriptName=DataPointDetailsDwr and c0-methodName=setPoint identify the control operation.

â€¢ c0-param0=number:2 identifies the writable point used by the challenge.

â€¢ c0-param2=boolean:false represents the OPEN command used in this scenario.

â€¢ HTTP/1.1 200 confirms that the Nginx/ScadaBR application accepted the request.


# **5. Live Modbus/TCP Process Traffic**

<img src="https://raw.githubusercontent.com/hacktify-uv/temp/main/ScadaBR-Feeder-HMI-Default-Operator-Login-Red-vs-Blue/blue/media/image4.png" style="width:9.45in;height:4.47748in" />

*Figure 4 â€” TCP/1502 decoded as Modbus/TCP, showing live process polling on the server loopback interface.*


**Observed**
â€¢ ScadaBR and the feeder process communicate over 127.0.0.1 because TCP/1502 is intentionally loopback-only.

â€¢ Function 1 reads coils, Function 2 reads discrete inputs and Function 4 reads input registers.

â€¢ The packets demonstrate continuous live interaction with the real Modbus feeder process.


# **6. Findings and Conclusion**

| **Evidence layer** | **What was observed** | **Validation status** |
|----|----|----|
| External HTTP | Kali-originated TCP/80 activity reached the target and invoked the ScadaBR DWR endpoint. | **Confirmed** |
| Authenticated application action | A valid JSESSIONID accompanied the setPoint request. The identifier is redacted from the report copy. | **Confirmed** |
| Control parameters | DataPointDetailsDwr.setPoint was called for point ID 2 with boolean false, representing the OPEN action. | **Confirmed** |
| Server response | The target returned HTTP/1.1 200 through Nginx. | **Confirmed** |
| OT process protocol | Live Modbus/TCP reads were decoded on localhost TCP/1502. | **Confirmed** |
| Explicit Modbus write packet | The supplied screenshots do not isolate a Function 5 Write Single Coil packet. | **Not claimed** |

**Conclusion**

The live demonstration satisfies the network-activity requirement. The Red Team script produced an observable external HTTP attack, authenticated to ScadaBR, invoked the breaker-control method and received a successful HTTP 200 response. The target server simultaneously showed active Modbus/TCP communication with the feeder process on loopback TCP/1502. Together, these artifacts establish a clear application-to-OT network evidence chain.

| **Evidence handling:** The screenshots embedded in this document have the transient JSESSIONID redacted. The original capture should be retained only in the controlled validation environment. |
|----|

**Recommended final validation step**

For a complete packet-level command chain, repeat a short clean capture and isolate modbus.func_code == 5 to document the Write Single Coil request, when present.
